...
**Token Optimization**: Scraper-mcp returns full page content (5-50k tokens) into main context. Delegate to throwaway sub-agent:
```
Task(subagent_type="general-purpose", model="haiku",
     prompt="Scrape <URL> using mcp__scraper__scrape_url with render_js=true. Return only: <specific info needed>.")
```
**Rationale**: Same pattern as Serena project activation — latency is worth the token savings. Sub-agent consumes full page in its context, returns only relevant extract to yours.
...
**Reference**: See ~/.claude/skills/theater-detection/ for detection methodology
...
**Cloud Run Deployment** (GitHub Actions Three-Tier Workflow):
- **When user says**: "Deploy to Cloud Run" or "Deploy AI Panel to cloud"
- **Action**: Orchestrate three-tier GitHub Actions workflow:
  1. Push v3.XX tag: `git tag v3.XX && git push origin v3.XX`
  2. Monitor ephemeral workflow (automatic via canary-deploy.yml)
  3. Trigger production: `gh workflow run production-deploy.yml -f source_tag=v3.XX`
  4. **STOP** - Tell user to run `./approve-deploy.sh <issue-number>` with Touch ID
  5. Monitor deployment after human approval
- **CRITICAL**: Agent CANNOT provide Touch ID - biometric gate requires human
